...
CREATE TABLE IF NOT EXISTS Dispositivos (
  ...
  estado TEXT NOT NULL DEFAULT 'ACTIVO' CHECK (estado IN ('ACTIVO','INACTIVO')),
  usuario_asociado_id UUID REFERENCES Usuarios(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS Servicios (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tipo TEXT NOT NULL UNIQUE DEFAULT 'SEGUIMIENTO CARDIACO Y RESPIRATORIO' CHECK (tipo IN ('SEGUIMIENTO CARDIACO Y RESPIRATORIO','ALERTA LADRIDO','NIVEL ACTIVIDAD','MEDICIONES ACTIVIDAD Y SUEÑO','LOCALIZACION Y ALERTA ANTIFUGA','CLIMATIZACION','IDENTIFICACION RFID','LOCALIZACION GPS','CONTROL TEMPERATURA','CONTROL CALIDAD AGUA','PATRONES RUMIA','INGESTA ALIMENTOS','ALERTA MASTITIS BOVINA','ALERTA PARTO','ESPECIALIZADO','TODOS','OTRO')),
...
DROP TABLE IF EXISTS DatosIoT CASCADE;
...
-- Servicio Contratado
DROP TABLE IF EXISTS ServicioContratado CASCADE;
...
-- Usuarios Invitados
DROP TABLE IF EXISTS UsuariosInvitados CASCADE;
...
-- Dueño de Animales
DROP TABLE IF EXISTS DueñoAnimal CASCADE;
...
DROP TABLE IF EXISTS DispositivoAnimal CASCADE; 
...
DROP TABLE IF EXISTS DatosIotDispositivo CASCADE;
...
DROP TABLE IF EXISTS DueñoDispositivo CASCADE;
